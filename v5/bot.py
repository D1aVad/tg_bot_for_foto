...
def get_options_keyboard():
    ...
    keyboard.add(pixelate_btn, ascii_btn, negative_btn, vertical_reflection_btn, horizontal_reflection_btn,
                 heat_map_btn, resize_for_stiker_btn)
    return keyboard
...
def resize_for_sticker_and_save(message, max_dimension=512):
    ...
    if width > max_dimension or height > max_dimension:
        ...
        output_stream = io.BytesIO()
        image = image.resize((new_width, new_height), Image.LANCZOS) # .LANCZOS - Этот фильтр обеспечивает высокое качество снижения размера изображения, уменьшая артефакты и предполагая более гладкие переходы между пикселями.
